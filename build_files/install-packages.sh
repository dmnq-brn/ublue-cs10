...
dnf -y --setopt=install_weak_deps=False install "${SHARED_PACKAGES[@]}" "${GUEST_AGENTS_PACKAGES[@]}" "${OPENSSH_SERVER_PACKAGES[@]}"

# remove unwanted packages
readarray -t INSTALLED < <(rpm -qa --queryformat='%{NAME}\n' "${UNVANTED_PACKAGES[@]}" "${GNOME_DESKTOP_PACKAGES[@]}" "${BLUETOOTH_PACKAGES[@]}" "${GNOME_BLUETOOTH_PACKAGES[@]}" "${WIFI_PACKAGES[@]}" "${OPENSSH_CLIENT_PACKAGES[@]}" "${QEMU_PACKAGES[@]}" "${DEVELOPMENT_PACKAGES[@]}" 2>/dev/null || true)
